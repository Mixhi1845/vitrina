...
export async function NetflixMovie() {
  const file = await fs.readFile(
    process.cwd() + "/src/lib/json/netflix_movie.json",
    "utf8"
  );
  const data = JSON.parse(file);
  const maxviews = (data.data[0].views / 1000000).toFixed(1);

  return (
    <div className="h-full flex-1 flex-col space-y-8 md:flex">
      <div className="w-full">
        <div className="flex items-center">
          <h2 className="text-2xl">Movies - {data.data_title}</h2>
